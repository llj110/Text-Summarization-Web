...
from flask import Flask, render_template, request, send_from_directory, url_for, make_response, send_file
...
def test():
    config = args
    textData = request.form["message1"]
    result = ""
    if textData != "":
        Best_Model = torch.load(config.test_model)
        Tokenizer = BertTokenizer.from_pretrained(config.model)
        ...
        # for idx, line in enumerate(f_in):
        source_ = f_in.strip().split()
        source = Tokenizer.tokenize(f_in.strip())
        mapping = mapping_tokenize(source_, source)
        ...
        pass
    return render_template('index.html', text=textData, result=result)
...
def upload_file():
    ...
    file_path = filedialog.askopenfilename(title=u'选择文件', filetypes=[("txt", ".txt")], initialdir=(os.path.expanduser(default_dir)))

    if file_path == '':
        pass
    else:
        messagebox.showinfo("提示","文本处理中...")
        config = args
        Best_Model = torch.load(config.test_model)
        Tokenizer = BertTokenizer.from_pretrained(config.model)
        ...
        result = open("D:/result.txt", 'w', encoding='utf-8')
        ed = '\n------------------------\n'
...
@app.route('/download')
def download():
    response = make_response(send_file("D:/result.txt"))
